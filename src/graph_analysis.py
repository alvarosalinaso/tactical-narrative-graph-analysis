...
def run_graph_analysis(data_dir: Path = Path("data/export"), output_dir: Path = Path("data/export")) -> dict:
    """
    ...
    for csv_file in csv_files:
        ...
        metrics = {
            ...
            "is_connected": nx.is_weakly_connected(G) if nx.is_weakly_connected else nx.is_connected(G),
        }
        ...
        metrics["top_betweenness"] = [{"player": p, "score": round(s, 4)} for p, s in top_betweenness]
        metrics["top_pagerank"] = [{"player": p, "score": round(s, 4)} for p, s in top_pagerank]
        metrics["top_degree"] = [{"player": p, "score": round(s, 4)} for p, s in top_degree]
        ...
        pagerank_data = [{"player": p, "pagerank": round(s, 4)} for p, s in sorted(pagerank.items(), key=lambda x: x[1], reverse=True)]
        ...
        print(f"[GRAPH] {name}: {metrics['nodes']} nodos, {metrics['edges']} aristas, {metrics.get('num_communities', '?')} comunidades")
        print(f"  Top betweenness: {top_betweenness[0][0]} ({top_betweenness[0][1]:.3f})")
